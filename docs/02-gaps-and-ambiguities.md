...
- **GAP-01 (Blocker) — Largely resolved 2026-08-18.** The four source
  documents were referenced but never supplied; only the prompt author's
  summary was available when this deliverable was written. The client has
  since supplied the actual IT Hardware Specifications Matrix, both SOPs
  (Issuance and Return), and `DEVICE ASSESSMENT FORM.docx` itself (verified
  directly, not a paraphrase) — recorded in `22-sapphire-virtual-source-data.md`.
  Its 8-point checklist / 3-outcome scheme has been reconciled against the
  already-implemented 23-point / 4-outcome system (see
  `ReconcileDeviceAssessmentChecklist1721200000000`) — that conflict is now
  closed. `ASSET ALLOCATION AND RETURN FORM.docx` also exists on the client's
  side but has **not yet been reviewed/diffed** against this deliverable's
  allocation/return field assumptions — that piece of GAP-01 remains open.
...
- **GAP-19 (Medium) — Resolved 2026-08-18.** No mention of bulk/legacy data
  import from the existing Excel sheets, yet "replace spreadsheet tracking"
  is a stated objective. The client has since supplied the full baseline
  workbook (136 laptop + 73 phone records) — see
  `22-sapphire-virtual-source-data.md` and
  `packages/backend/src/database/seeds/source-data/`, seeded via
  `seed-full-inventory.ts` (`npm run seed:inventory:full`). The source data
  itself is real-world messy (free-text status values, serials embedded in
  inconsistent formats) — see `inventory-xlsx-parser.ts` for the parsing
  heuristics and their documented low-confidence fallbacks; not every row's
  structured fields are high-confidence, but every row is imported and every
  row's raw source fields are preserved verbatim in its `notes`.
