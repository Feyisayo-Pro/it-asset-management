...
describe('BulkImportAssetsUseCase', () => {
  const csv = [
    ...
    'AST-CUSTOM-01,Laptop,Acme,X1,SN2,,,', // invalid tag format
  ...
  it('rejects an imported row whose asset tag does not match AST-{YYYY}-{SEQ}', async () => {
    const ctx = build();
    const result = await ctx.useCase.execute({ csv, dryRun: true });
    const badRow = result.results.find((r) => r.row === 3); // the AST-CUSTOM-01 row
    expect(badRow?.status).toBe('error');
    expect(badRow?.message).toContain('AST-{YYYY}-{SEQ}');
  });
  ...
  it('commit writes only the valid rows', async () => {
    ...
    expect(result.successCount).toBe(2);
    expect(ctx.assets.assets.size).toBe(2);
  ...
  it('rolls back all writes if the transaction fails partway through commit', async () => {
    const ctx = build();
    const originalAppend = ctx.assets.appendStatusHistory.bind(ctx.assets);
    let calls = 0;
    ctx.assets.appendStatusHistory = async (entry) => {
      calls += 1;
      if (calls === 2) throw new Error('simulated DB failure');
      return originalAppend(entry);
    };

    await expect(ctx.useCase.execute({ csv, dryRun: false })).rejects.toThrow(
      'simulated DB failure',
    );
    expect(ctx.assets.assets.size).toBe(0);
    expect(ctx.assets.history).toHaveLength(0);
    expect(ctx.events.namesOf()).not.toContain('asset.bulk-imported');
  });
});
