...
describe('AssessmentRecord entity', () => {
  ...
  it('upserts results — saving the same item twice keeps one entry', () => {
    const r = build();
    r.saveResults(template, [{ itemCode: 'screen_intact', result: 'Pass' }], nextId, now);
    r.saveResults(
      template,
      [{ itemCode: 'screen_intact', result: 'Fail', note: 'dead pixels' }],
      nextId,
      now,
    );
  ...
  it('refuses completion while required items are unanswered', () => {
    ...
    expect(() =>
      r.complete(
        template,
        { outcome: 'NoFaultFound', findings: 'ok', signatureName: 'Tech' },
        now,
      ),
  ...
  it('refuses completion when a failed item has no note', () => {
    const r = build();
    answerAll(r);
    r.saveResults(template, [{ itemCode: 'charger_available', result: 'Fail' }], nextId, now);
    expect(() =>
      r.complete(
        template,
        {
          outcome: 'RepairRecommended',
          findings: 'charger bad',
          recommendations: 'replace charger',
          signatureName: 'Tech',
        },
        now,
      ),
  ...
  it('requires recommendations for non-NoFaultFound outcomes', () => {
    const r = build();
    answerAll(r);
    expect(() =>
      r.complete(
        template,
        { outcome: 'ReplacementRecommended', findings: 'unusable', signatureName: 'Tech' },
        now,
      ),
  ...
  it('requires a technician signature', () => {
    ...
    expect(() =>
      r.complete(
        template,
        { outcome: 'NoFaultFound', findings: 'all good', signatureName: '  ' },
        now,
      ),
  ...
  it('completes with a full checklist and freezes the record', () => {
    ...
    expect(() =>
      r.saveResults(template, [{ itemCode: 'screen_intact', result: 'Fail', note: 'x' }], nextId, now),
    ).toThrow(AssessmentAlreadyCompletedError);
    expect(() =>
      r.complete(
        template,
        { outcome: 'NoFaultFound', findings: 'again', signatureName: 'T' },
        now,
      ),
    ).toThrow(AssessmentAlreadyCompletedError);
  });
